...
/// C3. Тест валидации Unicode
///
/// Проверка что невалидный UTF-8 отклоняется и эмодзи обрабатываются.
///
/// # Исправление C3 (CRITICAL)
/// Расширенная валидация Unicode для имён игроков с запретом
/// bidirectional control characters, zero-width joiners и variation selectors.
#[test]
fn test_unicode_validation() {
    use tetris_cli::highscore::sanitize::sanitize_player_name;

    // Тест 1: Эмодзи отфильтровываются
    let name_with_emoji = "Player😀Name";
    let sanitized = sanitize_player_name(name_with_emoji);
    assert!(
        !sanitized.contains('😀'),
        "Эмодзи должны быть отфильтрованы"
    );
    assert_eq!(sanitized, "PlayerName", "Остаются только валидные символы");

    // Тест 2: Bidirectional control characters отклоняются
    let name_with_bidi = "Player\u{200E}Name"; // U+200E - LTR mark
    let sanitized_bidi = sanitize_player_name(name_with_bidi);
    assert!(
        !sanitized_bidi.contains('\u{200E}'),
        "Bidi символы должны быть отфильтрованы"
    );

    // Тест 3: Zero-width joiners отклоняются
    let name_with_zwj = "Player\u{200D}Name"; // U+200D - ZWJ
    let sanitized_zwj = sanitize_player_name(name_with_zwj);
    assert!(
        !sanitized_zwj.contains('\u{200D}'),
        "Zero-width joiners должны быть отфильтрованы"
    );

    // Тест 4: Variation selectors отклоняются
    let name_with_variation = "Player\u{FE0F}Name"; // U+FE0F - Variation Selector-16
    let sanitized_variation = sanitize_player_name(name_with_variation);
    assert!(
        !sanitized_variation.contains('\u{FE0F}'),
        "Variation selectors должны быть отфильтрованы"
    );

    // Тест 5: Валидные Unicode символы (кириллица) разрешены
    ...
    // Тест 6: Только невалидные символы -> fallback на "Anonymous"
    let only_invalid = "😀🎉🚀";
...
/// M2. Тест централизации констант
///
/// Проверка что FRAME_DELAY_MS импортируется из constants.rs.
///
/// # Исправление M2 (MEDIUM)
/// Централизация всех констант в модуле constants.rs для устранения
/// дублирования и улучшения поддерживаемости.
#[test]
fn test_constants_centralization() {
    ...
    if Path::new(game_mod_path).exists() {
        ...
        if game_content.contains("const FRAME_DELAY_MS") {
            assert!(
                game_content.contains("use crate::constants") || game_content.contains("pub use constants"),
                "game/mod.rs должен импортировать FRAME_DELAY_MS из constants.rs, а не определять заново"
            );
        }
    }
}

/// M4. Тест #[must_use] атрибутов
///
/// Проверка что #[must_use] присутствует на критических функциях.
///
/// # Исправление M4 (MEDIUM)
/// Добавление #[must_use] атрибутов на функции которые должны быть
/// использованы для предотвращения ошибок.
#[test]
fn test_must_use_attributes() {
    ...
    assert!(
        name_content.contains("pub fn sanitize_player_name") && name_content.contains("-> String"),
        "sanitize_player_name должен возвращать String"
    );
}
