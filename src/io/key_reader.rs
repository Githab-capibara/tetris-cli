...
impl Drop for KeyReader {
    ...
    ///
    /// # Примечание о двойном вызове
    /// `Canvas`/`TermionBackend` также показывают курсор в своём `Drop`.
    /// Двойной вызов `Show` не является проблемой — терминал корректно
    /// обрабатывает повторные последовательности показа курсора.
    fn drop(&mut self) {
        use std::io::stdout;
        ...
        if let Err(_e) = write!(out, "{Show}") {
            crate::log_error!("Не удалось показать курсор в KeyReader::Drop");
        }
        if let Err(_e) = out.flush() {
            crate::log_error!("Не удалось сбросить буфер в KeyReader::Drop");
        }
    }
}
