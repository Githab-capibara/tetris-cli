...
fn test_edge_cases_1000_plus_lines() {
    ...
    assert!(state.get_lines_cleared() >= 1000, "Должно быть удалено 1000+ линий");
    
    // Проверяем что уровень корректно рассчитан
    let expected_level = (1000 / 10) + 1;
    assert!(state.get_level() >= expected_level, "Уровень должен быть рассчитан корректно");
}
...
fn test_min_field_size() {
    ...
    assert_eq!(blocks.len(), GRID_HEIGHT, "Высота поля должна быть {}", GRID_HEIGHT);
    assert_eq!(blocks[0].len(), GRID_WIDTH, "Ширина поля должна быть {}", GRID_WIDTH);
}
...
fn test_long_stability_1000_iterations() {
    ...
    for i in 0..1000 {
        ...
        assert!(state.get_curr_shape().coords.len() == 4, 
            "Итерация {}: фигура должна иметь 4 блока", i);
