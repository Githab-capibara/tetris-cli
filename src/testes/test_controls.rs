...
fn test_controls_default_config_values() {
    ...
    assert_eq!(config.move_left, b'a', "Движение влево должно быть 'a' (97)");
    assert_eq!(config.move_right, b'd', "Движение вправо должно быть 'd' (100)");
    assert_eq!(config.soft_drop, b's', "Мягкое падение должно быть 's' (115)");
    assert_eq!(config.hard_drop, b'w', "Жёсткое падение должно быть 'w' (119)");
    assert_eq!(config.rotate_left, b'q', "Вращение влево должно быть 'q' (113)");
    assert_eq!(config.rotate_right, b'e', "Вращение вправо должно быть 'e' (101)");
...
fn test_controls_default_trait() {
    ...
    assert_eq!(config_default.move_left, config_explicit.move_left,
               "Default должен совпадать с default_config() для move_left");
    assert_eq!(config_default.move_right, config_explicit.move_right,
               "Default должен совпадать с default_config() для move_right");
    assert_eq!(config_default.soft_drop, config_explicit.soft_drop,
               "Default должен совпадать с default_config() для soft_drop");
    assert_eq!(config_default.hard_drop, config_explicit.hard_drop,
               "Default должен совпадать с default_config() для hard_drop");
    assert_eq!(config_default.rotate_left, config_explicit.rotate_left,
               "Default должен совпадать с default_config() для rotate_left");
    assert_eq!(config_default.rotate_right, config_explicit.rotate_right,
               "Default должен совпадать с default_config() для rotate_right");
    assert_eq!(config_default.hold, config_explicit.hold,
               "Default должен совпадать с default_config() для hold");
    assert_eq!(config_default.pause, config_explicit.pause,
               "Default должен совпадать с default_config() для pause");
    assert_eq!(config_default.quit, config_explicit.quit,
    ...
    for i in 0..keys.len() {
        for j in (i + 1)..keys.len() {
            assert_ne!(keys[i], keys[j], 
                      "Клавиши под индексами {} и {} не должны совпадать", i, j);
        }
    }
}
...
fn test_controls_validation_valid_config() {
    let config = ControlsConfig::default_config();
    assert!(config.validate(), "Конфигурация по умолчанию должна быть валидной");
}
...
fn test_controls_validation_duplicate_keys() {
    ...
    assert!(!config_dup1.validate(), "Конфигурация с дубликатом move_left/move_right должна быть невалидной");
    
    // Тест с дубликатом rotate_left и rotate_right
    let config_dup2 = ControlsConfig::custom(
        b'a', b'd', b's', b'w', b'q', b'q', b'c', b'p', 127,
    );
    assert!(!config_dup2.validate(), "Конфигурация с дубликатом rotate_left/rotate_right должна быть невалидной");
    
    // Тест с дубликатом hold и pause
    let config_dup3 = ControlsConfig::custom(
        b'a', b'd', b's', b'w', b'q', b'e', b'c', b'c', 127,
    );
    assert!(!config_dup3.validate(), "Конфигурация с дубликатом hold/pause должна быть невалидной");
}
...
fn test_controls_validation_zero_values() {
    ...
    let config_zero1 = ControlsConfig::custom(
        0, b'd', b's', b'w', b'q', b'e', b'c', b'p', 127,
    );
    assert!(!config_zero1.validate(), "Конфигурация с нулевым move_left должна быть невалидной");
    
    // Тест с нулевым quit
    let config_zero2 = ControlsConfig::custom(
        b'a', b'd', b's', b'w', b'q', b'e', b'c', b'p', 0,
    );
    assert!(!config_zero2.validate(), "Конфигурация с нулевым quit должна быть невалидной");
    
    // Тест с несколькими нулями
    let config_zero3 = ControlsConfig::custom(
        0, b'd', 0, b'w', b'q', b'e', b'c', b'p', 0,
    );
    assert!(!config_zero3.validate(), "Конфигурация с несколькими нулями должна быть невалидной");
}
...
fn test_controls_validation_custom_valid() {
    ...
    assert!(vim_config.validate(), "Vim конфигурация должна быть валидной");
    
    // Конфигурация с цифрами
    let numpad_config = ControlsConfig::custom(
        b'1', b'2', b'3', b'4', b'5', b'6', b'7', b'8', b'9',
    );
    assert!(numpad_config.validate(), "Конфигурация с цифрами должна быть валидной");
    
    // Конфигурация со специальными символами
    let special_config = ControlsConfig::custom(
        b'[', b']', b';', b'\'', b',', b'.', b'/', b'\\', b'`',
    );
    assert!(special_config.validate(), "Конфигурация со спецсимволами должна быть валидной");
}
...
fn test_controls_validation_max_values() {
    ...
    let max_config = ControlsConfig::custom(
        248, 249, 250, 251, 252, 253, 254, 255, 247,
    );
    assert!(max_config.validate(), "Конфигурация с максимальными значениями должна быть валидной");
}
...
fn test_controls_save_to_file() {
    ...
    assert!(save_result.is_ok(), "Сохранение должно быть успешным: {:?}", save_result.err());
    
    // Проверяем, что файл существует
    assert!(Path::new(test_path).exists(), "Файл конфигурации должен существовать после сохранения");
...
fn test_controls_load_from_file() {
    ...
    assert!(loaded_result.is_ok(), "Загрузка должна быть успешной: {:?}", loaded_result.err());
    
    let loaded_config = loaded_result.unwrap();
    assert_eq!(original_config, loaded_config, "Загруженная конфигурация должна совпадать с оригиналом");
...
fn test_controls_load_nonexistent_file() {
    ...
    assert!(result.is_err(), "Загрузка из несуществующего файла должна возвращать ошибку");
}
...
fn test_controls_save_load_cycle() {
    ...
    let original = ControlsConfig::custom(
        b'1', b'2', b'3', b'4', b'5', b'6', b'7', b'8', b'9',
    );
    
    // Сохраняем
    assert!(original.save_to_file(test_path).is_ok(), "Сохранение должно быть успешным");
    
    // Загружаем
    let loaded = ControlsConfig::load_from_file(test_path).expect("Загрузка должна быть успешной");
    
    // Сравниваем все поля
    assert_eq!(original.move_left, loaded.move_left, "move_left должен совпадать");
    assert_eq!(original.move_right, loaded.move_right, "move_right должен совпадать");
    assert_eq!(original.soft_drop, loaded.soft_drop, "soft_drop должен совпадать");
    assert_eq!(original.hard_drop, loaded.hard_drop, "hard_drop должен совпадать");
    assert_eq!(original.rotate_left, loaded.rotate_left, "rotate_left должен совпадать");
    assert_eq!(original.rotate_right, loaded.rotate_right, "rotate_right должен совпадать");
...
fn test_controls_save_load_special_chars() {
    ...
    assert_eq!(original, loaded, "Конфигурация со спецсимволами должна сохраняться корректно");
...
fn test_controls_custom_vim_style() {
    ...
    assert_eq!(vim_config.move_right, b'l', "Vim move_right должен быть 'l'");
...
fn test_controls_custom_numpad_style() {
    ...
    assert!(numpad_config.validate(), "Нумпад конфигурация должна быть валидной");
}
...
fn test_controls_custom_differs_from_default() {
    ...
    let differs = 
        default_config.move_left != custom_config.move_left ||
    ...
    assert!(differs, "Кастомная конфигурация должна отличаться от стандартной");
}
...
fn test_controls_boundary_min_values() {
    ...
    let min_config = ControlsConfig::custom(
        1, 2, 3, 4, 5, 6, 7, 8, 9,
    );
    assert!(min_config.validate(), "Конфигурация с минимальными значениями должна быть валидной");
    
    // Конфигурация с нулём должна быть невалидной
    let zero_config = ControlsConfig::custom(
        0, 2, 3, 4, 5, 6, 7, 8, 9,
    );
    assert!(!zero_config.validate(), "Конфигурация с нулевым значением должна быть невалидной");
}
