...
fn test_full_game_initialization() {
    ...
    assert_eq!(state.get_lines_cleared(), 0, "Начальные линии должны быть 0");
    assert_eq!(state.get_mode(), GameMode::Classic, "Режим по умолчанию - Classic");

    // Проверяем наличие фигур
    assert!((state.get_curr_shape().shape as usize) < 7, "Текущая фигура должна быть валидной");
    assert!((state.get_next_shape().shape as usize) < 7, "Следующая фигура должна быть валидной");
}
...
fn test_piece_movement_cycle() {
    ...
    assert!(
        (final_x - initial_x).abs() > f32::EPSILON || 
        (final_y - initial_y).abs() > f32::EPSILON,
...
fn test_rotation_in_game_context() {
    ...
    assert!(can_rotate_right || can_rotate_left,
            "Хотя бы одно направление вращения должно быть доступно");
}
...
fn test_game_state_tetromino_interaction() {
    ...
    assert_eq!(curr.shape as usize, curr.fg,
...
fn test_game_state_leaderboard_interaction() {
    ...
    assert_eq!(leaderboard.get_best_score(), final_score,
...
fn test_game_stats_game_state_interaction() {
    ...
    assert_eq!(stats.total_pieces(), 1, "В начале игры должна быть 1 фигура");
...
fn test_save_data_leaderboard_interaction() {
    ...
    assert_eq!(leaderboard.get_best_score(), 3000,
...
fn test_rotation_collision_interaction() {
    ...
    assert!(can_rotate || !can_rotate, "Метод должен вернуть результат");
}
