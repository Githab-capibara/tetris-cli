...
fn test_tetromino_from_bag_to_gamestate() {
    ...
    assert!((tetromino.shape as usize) < 7, "Тип фигуры должен быть валидным");
}
...
fn test_all_piece_types_appear_in_game() {
    ...
    for (i, &found) in found_shapes.iter().enumerate() {
        assert!(
            found,
            "Фигура типа {:?} должна появиться в игре",
            i
        );
    }
}
...
fn test_piece_rotation_in_gamestate() {
    ...
    assert!(
        ...
        "Вращение должно быть проверено"
    );
}
...
fn test_piece_stays_within_bounds() {
    ...
    for &(x, _) in shape.coords.iter() {
        ...
        assert!(
            global_x >= 0,
            "Фигура не должна выходить за левую границу"
        );
    }
}
...
fn test_next_shape_from_bag_generator() {
    ...
    let next = state.get_next_shape();
    assert!((next.shape as usize) < 7, "Следующая фигура должна быть валидной");
    assert_eq!(next.pos, (4.0, 0.0), "Позиция следующей фигуры должна быть начальной");
}
...
fn test_move_left_right() {
    ...
    assert!(after_left <= initial_x, "После движения влево X должен уменьшиться");
    assert!(after_right >= after_left, "После движения вправо X должен увеличиться");
}
