...
fn test_unwrap_or_default_is_safe() {
    ...
    assert_eq!(default_from_none, 0, "unwrap_or_default для None должен вернуть 0");
    
    // unwrap_or_default() возвращает значение для Some
    let value_from_some = some_value.unwrap_or_default();
    assert_eq!(value_from_some, 42, "unwrap_or_default для Some должен вернуть значение");
}
...
fn test_enumerate_gives_index_and_value() {
    ...
    for (index, &value) in values.iter().enumerate() {
        ...
        assert_eq!(values[index], value, "Индекс должен соответствовать значению");
    }
...
fn test_take_limits_iteration() {
    ...
    assert_eq!(first_three, vec![1, 2, 3], "take(3) должен взять 3 элемента");
...
fn test_range_contains_works() {
    ...
    // Проверяем不包含ение
    assert!(!(0..=10).contains(&-1), "-1 не должен быть в диапазоне 0..=10");
    assert!(!(0..=10).contains(&11), "11 не должен быть в диапазоне 0..=10");
}
...
fn test_contains_more_readable_than_comparison() {
    ...
    assert_eq!(contains_result, comparison_result, "Результаты должны совпадать");
...
fn test_to_string_preferred_for_simple_strings() {
    ...
    assert_eq!(string1, string2, ".to_string() и format! должны быть одинаковы");
    assert_eq!(string1, string3, ".to_string() и format!(s) должны быть одинаковы");
...
fn test_try_from_safer_than_as() {
    ...
    assert!(result.is_err(), "try_from должен вернуть ошибку для большого значения");
