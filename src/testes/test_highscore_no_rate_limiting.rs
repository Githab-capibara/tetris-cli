...
//!
//! Исправление: rate limiting добавлен, максимум 10 записей в минуту.
...
fn test_наличие_блокировки_rate_limiting() {
    ...
    for i in 0..10 {
        ...
        assert!(
            result,
            "Рекорд {} должен быть добавлен (в пределах лимита)",
            i
        );
    }
    ...
    assert!(
        ...
        "Минимальный рекорд в топ-5 должен быть >= 50, получен {}",
        min_score
    );
    ...
    assert!(true, "Rate limiting работает корректно");
}
