...
///
/// ## Исправление аудита 2026-04-14 (LOW #23)
/// Добавлен `PartialEq` для возможности сравнения фигур в тестах через `assert_eq!`.
///
...
pub struct Tetromino {
    ...
    ///
    /// ## Почему f32 вместо целочисленной сетки?
    /// Позиция хранится как (f32, f32) для поддержки плавного падения фигур
    /// с интерполяцией между кадрами. Это позволяет:
    /// - Плавную анимацию падения с дробными координатами
    /// - Корректную работу wall kick с дробными смещениями
    /// - Интерполяцию позиции между кадрами при 60 FPS
    ///
    /// При сохранении фигуры в сетку координаты округляются до целых (as i16).
...
impl Tetromino {
    ...
    pub fn rotate(&mut self, dir: RotationDirection) -> bool {
        ...
        //
        // Исправление атомарности: сначала вычисляем все новые координаты во временный массив.
        // Только если все 4 прошли проверку — применяем их. Это предотвращает
