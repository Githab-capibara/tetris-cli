...
impl Drop for KeyReader {
    ...
    ///
    /// # Исправление аудита 2026-03-31 (CRITICAL)
    /// async_stdin автоматически управляет raw-режимом. При уничтожении KeyReader
    /// raw-режим отключается автоматически. Явный вызов не требуется.
    fn drop(&mut self) {
        // Примечание: async_stdin автоматически управляет raw-режимом.
        // При уничтожении KeyReader raw-режим отключается автоматически.
        ...
        match self.inp.read_exact(&mut key_bytes) {
            Ok(()) => {
                ...
                let bytes_to_read = if (0xC2..=0xDF).contains(&first_byte) {
                    ...
                    // Исправление аудита 2026-03-31: невалидный байт тихо игнорируется
                    // eprintln предупреждения удалены для чистоты вывода
                    return Ok(None);
                };
                ...
                if self
                    ...
                    .is_err()
                {
                    // Исправление аудита 2026-03-31: ошибка чтения тихо игнорируется
                    // eprintln предупреждения удалены для чистоты вывода
                    return Ok(None);
                }
                ...
                if std::str::from_utf8(&utf8_bytes[..=bytes_to_read]).is_err() {
                    ...
                    // Исправление аудита 2026-03-31: логирование только для отладки
                    eprintln!(
                ...
                // Исправление аудита 2026-03-31: убрано eprintln предупреждение для UTF-8
                // Многобайтовые символы UTF-8 теперь тихо игнорируются так как они
                // не являются управляющими клавишами для игры.
                // Для поддержки Unicode используйте get_key_unicode().
