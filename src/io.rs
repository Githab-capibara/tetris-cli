...
impl KeyReader {
    ...
    ///
    /// # Отличия от get_key()
    /// - `get_key()` возвращает `u8` и поддерживает только ASCII (0x00-0x7F)
    /// - `get_key_unicode()` возвращает `char` и поддерживает полный Unicode
    ///
    /// # Примечания
    /// - Для специальных клавиш (стрелки, Home, End) может возвращать `None`
    ///   или первый символ ESC-последовательности
    /// - Метод читает все байты символа UTF-8 и декодирует их
    ///
    /// # Исправление #13
    /// Добавлена поддержка UTF-8 для локализации управления.
