...
///
/// # Defence in depth
/// Используются две разных стратегии защиты от переполнения:
/// - `MAX_SCORE = u128::MAX / 2` — лимит на уровне бизнес-логики, оставляющий запас
...
pub fn check_rows(state: &mut GameState) -> u32 {
    ...
    if remove_count > 0 {
        ...
        // Игнорируем ошибки bell-сигнала — не критичная операция
