...
/// Каждый обработчик получает события через метод `handle()` и может
/// выполнять соответствующие действия (обновление счёта, сохранение статистики и т.д.).
///
/// ## Архитектурные заметки
/// ## Исправление C1 (CRITICAL): Событийная модель
...
pub trait EventHandler {
    ...
    ///
    /// Этот метод вызывается диспетчером событий для каждого подписчика
    /// при возникновении события. Реализация должна быть быстрой и не
    /// содержать блокирующих операций, чтобы не задерживать доставку
    /// событий другим подписчикам.
    ///
    /// # Аргументы
