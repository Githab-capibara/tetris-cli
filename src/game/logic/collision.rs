...
// В тестах координаты ограничены полем (10x20), cast безопасен.
#![allow(
    clippy::cast_possible_truncation,
    clippy::cast_possible_wrap,
    clippy::cast_sign_loss,
    reason = "Тестовые координаты ограничены размером поля (10x20)"
)]
...
fn is_position_valid<T: BoardReadonly>(
    ...
    // Блоки выше поля (Y < 0) обрабатываются в зависимости от ignore_above_field:
    // - true: игнорируем (фигуры могут находиться выше видимого поля, как при вращении)
    // - false: считаем коллизией (фигура не должна быть выше поля при движении вниз)
    if check_y < 0 {
        return ignore_above_field; // true = валидна, false = коллизия
    }
...
fn check_collision_direction<T: BoardReadonly>(
    ...
    coords.iter().any(|&(coord_x, coord_y)| {
        ...
        // Игнорируем блоки выше поля (Y < 0) для всех направлений — фигуры спавнятся
        // выше видимого поля и должны иметь возможность двигаться до входа в игровую зону
