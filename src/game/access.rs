...
pub trait BoardMutable: BoardReadonly {
    ...
    ///
    /// # Примечание
    /// `debug_assert` — это намеренный маркер "не реализовано" для опциональных методов трейта.
    /// Он срабатывает только в debug-режиме и сигнализирует если метод вызван для типа,
    /// который ДОЛЖЕН его переопределить (например, `GameState`). В release-режиме assert
    /// отсутствует и возвращается безопасное значение по умолчанию.
    #[must_use]
    #[inline]
    fn get_fall_speed(&self) -> f32 {
        debug_assert!(
            false,
            "get_fall_speed(): не реализовано — это маркер 'not implemented' для опционального метода трейта. \
             Если вызван для GameState — баг, иначе — ожидаемое поведение."
        );
        0.0
    }
    ...
    /// `debug_assert` — это намеренный маркер "не реализовано" для опциональных методов трейта.
    /// Он срабатывает только в debug-режиме и сигнализирует если метод вызван для типа,
    /// который ДОЛЖЕН его переопределить. В release-режиме assert отсутствует.
    #[must_use]
    #[inline]
    fn get_land_timer(&self) -> f64 {
        debug_assert!(
            false,
            "get_land_timer(): не реализовано — это маркер 'not implemented' для опционального метода трейта. \
             Если вызван для GameState — баг, иначе — ожидаемое поведение."
        );
        0.0
    }
...
pub trait ScoreAccess {
    ...
    /// Он срабатывает только в debug-режиме и сигнализирует если метод вызван для типа,
    /// который ДОЛЖЕН его переопределить (например, `GameState`). В release-режиме assert
    /// отсутствует и возвращается безопасное значение по умолчанию (0).
    #[must_use]
    #[inline]
    fn get_score(&self) -> u128 {
        debug_assert!(
            false,
            "get_score(): не реализовано — это маркер 'not implemented' для опционального метода трейта. \
             Если вызван для типа который должен иметь счёт — баг, иначе — ожидаемое поведение."
        );
        0
    }
}
