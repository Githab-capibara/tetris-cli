...
/// ```ignore
/// fn render_field<T: GameFieldAccess>(field: &T) {
...
/// Трейт для доступа к счёту и уровню.
///
/// Предоставляет контролируемый доступ к игровой статистике,
/// не раскрывая полное состояние игры.
///
/// ## Пример использования
/// ```ignore
/// fn display_score<T: ScoreAccess>(score: &T) {
///     println!("Счёт: {}", score.get_score());
///     println!("Уровень: {}", score.get_level());
/// }
/// ```
#[allow(dead_code)]
...
/// Трейт для доступа к скорости и таймерам.
///
/// Предоставляет доступ к параметрам физики игры.
#[allow(dead_code)]
pub trait GamePhysicsAccess {
