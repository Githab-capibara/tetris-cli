...
//! - [`view`] — представление игры для отрисовки ([`view::GameView`])
...
//! ```
//!
//! ## Архитектура: Граф зависимостей (ID 68)
//! Все зависимости — направленный ациклический граф (DAG). Циклических зависимостей НЕТ:
//! - Базовые модули (constants, types, `state`, `mode_trait`) не зависят от других game/*
//! - Трейты доступа (access) зависят от state, но state не зависит от access
//! - View, logic, scoring, render зависят от state, но не наоборот
//! - cycle.rs — orchestrator верхнего уровня, зависит от всех, но не вызывается из подмодулей
//!
//!   Компоненты (`components/`) выделены для предотвращения потенциальных циклов.
//!
//!   Если новый модуль создаёт цикл — это сигнал о нарушении архитектуры.
//!
//! ## Порядок инициализации
//! 1. `constants.rs` - централизованные константы
//! 2. `types.rs` - типобезопасные обёртки ([`types::Score`], [`types::Level`], [`types::LinesCount`])
...
// а не из вложенных модулей (например, `crate::game::state::GameState`).
// Не удалять — используется во внешнем API и интеграционных тестах.
