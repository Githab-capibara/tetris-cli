...
impl GameState {
    ...
    ///
    /// ## Намеренное использование строкового сравнения
    /// Обратное преобразование из имени режима в enum намеренно использует
    /// строковое сравнение для обеспечения обратной совместимости.
    /// Это позволяет добавлять новые режимы через GameModeTrait без
    /// необходимости изменять этот метод — новые режимы будут автоматически
...
mod state_tests {
    ...
    fn test_spawn_new_piece() {
        ...
        assert!(
            !shape.coords().is_empty(),
            "Фигура должна иметь координаты"
        );
    }
