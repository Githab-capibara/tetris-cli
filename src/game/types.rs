...
///
/// Используется `u128` для предотвращения переполнения счёта при длительных игровых
/// сессиях. При уровне 1000 и постоянном удалении линий (Tetris = 1800 очков × уровень)
/// счёт может превысить `u64::MAX` за ~10⁹ линий. `u128` гарантирует отсутствие
/// переполнения в любом реалистичном сценарии.
///
...
// Position больше не используется через этот модуль — ре-экспорт удалён
