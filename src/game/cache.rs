...
//!
//! ### Архитектурное решение (ID 79): UI-строки и отслеживание изменений
//! `RenderCache` намеренно объединяет две ответственности:
//! 1. Кэшированные строки для отрисовки (`cached_*_str`) — готовые форматированные строки
//! 2. Последние значения для сравнения (`last_cached_*`) — отслеживание изменений
//!
//!    Это прагматичное решение: разделение на два отдельных типа увеличило бы сложность
//!    без реальной выгоды, так как оба аспекта всегда используются вместе в render-цикле.
//!
//!    При рефакторинге render-системы рассмотреть разделение на `UiStringCache` и `ChangeTracker`.
...
impl RenderCache {
    ...
    ///
    /// # Производительность (P3-ID49)
    /// Все строки инициализируются с `String::with_capacity(32)`.
    /// Это небольшие stack-friendly аллокации:
    /// - Типичная строка счёта: ~10 байт (`"       100"`)
    /// - Типичная строка таймера: ~15 байт (`"     123.45s"`)
    /// - Ёмкость 32 байта покрывает все форматы с запасом
    ///   и остаётся в пределах small-string optimization большинства аллокаторов.
    ///
    /// # Исправление #4 (LOW)
