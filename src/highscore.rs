...
//!
//! В этом модуле используется `DefaultHasher` для вычисления хешей.
//! **DefaultHasher не является криптографически стойким** и предназначен только
//! для защиты от случайных подделок (например, если пользователь случайно отредактирует
//! файл конфигурации). Для серьёзной защиты от намеренных подделок следует использовать
//! криптографические хеш-функции (SHA-256, BLAKE3 и т.д.).
//!
//! ## Структура модуля
