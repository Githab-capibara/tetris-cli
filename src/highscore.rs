...
// ===========================================================================
// Структуры для сохранения и загрузки настроек управления

/// Конфигурация управления игрой.
///
/// Содержит коды клавиш для всех действий в игре.
/// Сохраняется в файле controls.toml в формате:
/// ```toml
/// left_key = 97
/// right_key = 100
/// rotate_left_key = 113
/// rotate_right_key = 101
/// hard_drop_key = 119
/// soft_drop_key = 115
/// hold_key = 99
/// pause_key = 112
...
pub struct ControlsConfig {
    /// Клавиша перемещения влево (по умолчанию 'a' = 97).
    pub left_key: u8,
    /// Клавиша перемещения вправо (по умолчанию 'd' = 100).
    pub right_key: u8,
    /// Клавиша вращения влево (по умолчанию 'q' = 113).
    pub rotate_left_key: u8,
    /// Клавиша вращения вправо (по умолчанию 'e' = 101).
    pub rotate_right_key: u8,
    /// Клавиша Hard Drop (по умолчанию 'w' = 119).
    pub hard_drop_key: u8,
    /// Клавиша Soft Drop (по умолчанию 's' = 115).
    pub soft_drop_key: u8,
    /// Клавиша удержания фигуры (по умолчанию 'c' = 99).
    pub hold_key: u8,
    /// Клавиша паузы (по умолчанию 'p' = 112).
    pub pause_key: u8,
}
...
impl ControlsConfig {
    /// Загрузить конфигурацию управления из файла.
    ///
    /// # Возвращает
    /// ControlsConfig по умолчанию при ошибке загрузки
    pub fn load() -> Self {
        load(&format!("{}_controls", APP_NAME)).unwrap_or_default()
    }

    /// Сохранить конфигурацию управления в файл.
    pub fn save(&self) {
        if let Err(e) = store(&format!("{}_controls", APP_NAME), self) {
            eprintln!("Ошибка сохранения конфигурации управления: {}", e);
        }
    }

    /// Создать новую конфигурацию с заданными клавишами.
    ///
    /// # Аргументы
    /// * `left_key` - клавиша влево
    /// * `right_key` - клавиша вправо
    /// * `rotate_left_key` - клавиша вращения влево
    /// * `rotate_right_key` - клавиша вращения вправо
    /// * `hard_drop_key` - клавиша Hard Drop
    /// * `soft_drop_key` - клавиша Soft Drop
    /// * `hold_key` - клавиша удержания
    /// * `pause_key` - клавиша паузы
    pub fn new(
        left_key: u8,
        right_key: u8,
        rotate_left_key: u8,
