...
impl PathValidator {
    ...
    /// Проверить отсутствие символических ссылок (свободная функция).
    ///
    /// # Аргументы
    /// * `path` - путь для проверки
    ///
    ...
    /// Возвращает `PathError` если путь является символической ссылкой или если родительская директория является symlink.
    ///
    /// # Исправление аудита 2026-04-13 (PROB-014)
    /// Выделено в отдельную функцию для устранения `unused_self`.
    #[must_use = "Результат валидации symlink должен быть обработан"]
    #[track_caller]
    pub fn check_no_symlinks(path: &Path) -> Result<(), PathError> {
    ...
    pub fn validate_within_directory(&self, path: &Path, dir: &Path) -> Result<(), PathError> {
        Self::check_within_directory(path, dir)
    }

    /// Проверить, что путь находится внутри директории (свободная функция).
    ///
    /// # Аргументы
    /// * `path` - путь для проверки
    /// * `dir` - разрешённая директория
    ...
    /// Выделено в отдельную функцию для устранения `unused_self`.
    #[must_use = "Результат валидации директории должен быть обработан"]
    pub fn check_within_directory(path: &Path, dir: &Path) -> Result<(), PathError> {
