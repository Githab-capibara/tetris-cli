//! Тесты столкновений.
//!
//! Модуль содержит 9 параметризированных тестов для проверки системы столкновений:
//! - Столкновения со стенами (2 теста)
//! - Столкновения с полом (2 теста)
//! - Столкновения-away from wall (2 теста)
//! - Столкновения всех фигур со стенами и полом (1 тест)
//! - Столкновения при движении с блоками (2 теста)
...
fn test_collision_rotation_o_piece() {
    let mut state = GameState::new();
    state.mutate_curr_shape(|s| {
        s.set_shape(ShapeType::O);
        s.set_coords(SHAPE_COORDS[ShapeType::O as usize]);
    });
...
fn test_collision_array_bounds() {
    ...
    // После движения к левой границе и вниз проверяем что левое направление заблокировано
    let can_left = state.can_move_curr_shape_direction(Direction::Left);
    // Фигура должна упираться в левую стену
    assert!(
        !can_left,
