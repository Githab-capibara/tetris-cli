...
fn test_track_caller_performance() {
    ...
    // 10000 итераций должны выполняться < 300ms (увеличенный таймаут для стабильности)
    assert!(
        elapsed.as_millis() < 300,
        "track_caller не должен значительно влиять на производительность (прошло {elapsed:?})"
    );
}
