...
//!
//! Модуль содержит 6 интеграционных тестов для проверки
//! взаимодействия всех компонентов игры:
//! - Полный игровой цикл (2 теста)
//! - Взаимодействие компонентов (2 теста)
//! - Расширенные интеграционные сценарии (2 теста)
...
fn test_sprint_game_initialization() {
    ...
    // Запускаем таймер и проверяем что он инициализируется корректно
    state.start_timer();

    // Сразу после запуска время должно быть 0 (или очень близко к 0)
    let elapsed = state.stats().get_elapsed_time();
    assert!(
        elapsed < 1.0,
        "Время сразу после запуска должно быть < 1 секунды (получено {elapsed})"
    );
}
