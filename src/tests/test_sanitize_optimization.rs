...
fn test_sanitize_performance() {
    ...
    // 10000 итераций × 5 имён должны выполняться < 2000ms (увеличенный таймаут для стабильности на медленных системах)
    assert!(
        elapsed.as_millis() < 2000,
        "Санитизация {iterations} итераций должна выполняться < 2000ms (прошло {:?})",
        elapsed
    );
}
