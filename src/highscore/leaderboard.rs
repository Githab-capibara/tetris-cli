...
///
/// ### Ограничения в многопоточном коде
/// - ⚠️ Не используйте `LeaderboardEntry` из нескольких потоков без синхронизации
/// - ⚠️ Метод `is_valid()` может устареть между вызовами
/// - ✅ Для многопоточного доступа используйте `Arc<Mutex<LeaderboardEntry>>`
///
...
/// let score = entry.lock().unwrap().score(); // Безопасно
/// ```
