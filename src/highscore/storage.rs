...
impl LeaderboardValidator {
    ...
    ///
    /// # Panics
    /// Не паникует. HMAC-SHA256 для валидных UTF-8 строк всегда успешен.
    ///
    /// # Исправление аудита 2026-04-11 (Пакет 1, #3)
    /// Добавлена безопасная обработка через `expect` с диагностическим сообщением.
    ///
    /// # Audit 2026-04-12, Issue #23
    /// Добавлена секция # Panics в документацию для явного указания безопасности.
    #[must_use]
    pub fn compute_signature(salt: &str, name: &str, score: u128) -> String {
        ...
        hmac_sign_with_salt(get_leaderboard_hmac_key(), salt, &salt_name_score)
            .expect("HMAC подпись для валидных UTF-8 входов не может вернуть ошибку")
    }
