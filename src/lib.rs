...
/// Модуль общих типов.
///
/// Содержит базовые типы, используемые несколькими модулями:
...
///
/// ## Подмодули:
/// - `state` — структуры состояния (GameState, GameStats, GameMode)
/// - `logic` — игровая логика (физика, коллизии, движение)
...
/// ## Пример использования:
/// ```ignore
/// use tetris_cli::app::{Application, run};
///
/// // Запуск приложения
/// run().expect("Ошибка запуска приложения");
///
/// // Или вручную:
/// let mut app = Application::new().expect("Ошибка инициализации");
/// app.run().expect("Ошибка выполнения");
/// ```
pub mod app;

/// Модуль меню.
///
/// Предоставляет функции для отображения главного меню и запуска режимов игры:
/// - `draw_menu()` — отрисовка главного меню
/// - `run_game_mode()` — запуск игрового режима
/// - `show_leaderboard()` — отображение таблицы лидеров
pub mod menu;
...
/// let high_score = save.verify_and_get_score();
///
/// // Таблица лидеров
...
/// Модуль абстракций ввода-вывода.
///
/// Предоставляет трейты для работы с вводом и выводом:
/// - `InputReader` — трейт для чтения нажатий клавиш
...
// Все экспорты теперь находятся в модуле exports.

// Экспорт модуля exports для удобного импорта
pub use exports::*;
