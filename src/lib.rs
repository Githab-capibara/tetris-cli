...
// Архитектурная заметка (ID 69-70): #[macro_use] для модулей макросов
// #[macro_export] макросы (log_error!, log_warn!, log_info!) должны быть видимы
// на уровне крейта до того, как другие модули начнут их использовать.
// #[macro_use] mod macros — обеспечивает видимость макросов во всех последующих модулях.
// #[macro_use] pub mod app — макросы, используемые внутри app, требуют #[macro_use]
// для корректного разрешения имён. Альтернатива с `use crate::...` не работает
// с #[macro_export], так как эти макросы разворачиваются на этапе компиляции.
#[macro_use]
mod macros;
