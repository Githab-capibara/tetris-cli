...
fn create_hmac_instance(key: &[u8]) -> HmacSha256 {
    ...
    HmacSha256::new_from_slice(key).unwrap_or_else(|e| {
        // Этот путь недостижим для HMAC-SHA256, но оставляем для полноты обработки
        panic!("Невозможная ошибка инициализации HMAC-SHA256: {e}");
    })
}
