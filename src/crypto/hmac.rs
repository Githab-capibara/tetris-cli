...
///
/// Вынесено в отдельную функцию для устранения дублирования кода.
#[inline]
fn create_hmac_instance(key: &[u8]) -> HmacSha256 {
    ...
    HmacSha256::new_from_slice(key).expect(
        "Невозможная ошибка инициализации HMAC-SHA256: ключ невалиден. \
         Согласно RFC 2104, HMAC-SHA256 принимает ключи любой длины. \
         Это указывает на критическое изменение в библиотеке hmac.",
    )
}
...
mod hmac_tests {
    ...
    // ========================================================================
    // Тесты: /VERIFY С СОЛЬЮ (исправление #59-60)
    // ========================================================================
