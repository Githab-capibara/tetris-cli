...
fn create_hmac_instance(key: &[u8]) -> HmacSha256 {
    ...
    // expect() используется вместо unwrap_or_else(unreachable!) для более явной семантики.
    HmacSha256::new_from_slice(key).expect("HMAC-SHA256 accepts keys of any length per RFC 2104")
}
