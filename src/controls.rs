...
/// Единая функция валидации пути конфигурации.
///
/// Эта функция объединяет все проверки валидации путей в одном месте:
/// 1. Проверка длины пути (максимум 255 символов)
/// 2. Проверка допустимых символов (запрещены \0, |, &, ;, $)
/// 3. Проверка на отсутствие абсолютных путей
/// 4. Проверка на отсутствие path traversal (..)
/// 5. Проверка на отсутствие символических ссылок
/// 6. Проверка нахождения пути внутри разрешённой директории
///
/// # Аргументы
/// * `path` - путь для комплексной валидации
///
/// # Возвращает
/// - `Ok(())` если все проверки пройдены
/// - `Err(io::Error)` если какая-либо проверка не пройдена
///
/// # Безопасность
/// Защищает от:
/// - Path traversal атак (через "..")
/// - Symlink атак (символические ссылки запрещены)
/// - Ввода абсолютных путей вне директории приложения
/// - Специальных символов для инъекций команд
///
/// # Пример использования
/// ```ignore
/// use tetris_cli::controls::validate_config_path_comprehensive;
/// validate_config_path_comprehensive("config.json").unwrap();
/// ```
///
/// TODO (#архитектура): Использовать эту функцию вместо validate_config_path()
/// для всей валидации путей в проекте.
#[allow(dead_code)] // Будет использована в будущем рефакторинге
fn validate_config_path_comprehensive(path: &str) -> io::Result<()> {
