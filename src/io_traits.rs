...
//!
//! ## Почему трейты вынесены в отдельный модуль
//! Трейты `InputReader` и `Renderer` намеренно вынесены в отдельный модуль `io_traits.rs`,
//! а не в `io/mod.rs`. Это позволяет импортировать трейты без зависимости от конкретных
//! реализаций (`Canvas`, `KeyReader`). Такое разделение полезно для:
//! - Тестирования: можно создать mock-реализации без зависимости от терминала
//! - Расширяемости: можно реализовать трейты для других бэкендов (crossterm, ncurses)
...
/// ## Пример реализации
///
/// Обратите внимание: примеры в этом модуле используют `ignore` в doc-тестах,
/// так как это CLI-приложение требует реального терминала для работы.
/// Примеры приведены для иллюстрации API и не могут быть запущены в рамках `cargo test`.
///
/// ```ignore
...
/// Примеры используют `ignore` так как требуют реального терминала (CLI-приложение).
///
...
pub trait Renderer {
    ...
    ///
    /// # Архитектурная заметка (ID 64)
    /// Дефолтная реализация намеренно делегирует `draw_strs` для обратной совместимости:
