...
//!
//! ## PROB-153: HMAC и защита от replay-атак
//! Для CLI-игры replay-атаки НЕ АКТУАЛЬНЫ — данные хранятся локально и не
//! передаются по сети. HMAC используется исключительно для проверки целостности
//! локальных файлов конфигурации. При добавлении сетевого режима потребуется
//! nonce/timestamp для защиты от replay.
//!
//! ## PROB-154: Таймаут на крипто-операции
//! Для CLI-игры таймауты НЕ АКТУАЛЬНЫ — все операции выполняются локально
//! и завершаются мгновенно. При сетевом взаимодействии — добавить таймауты
//! на HMAC вычисление и проверку.
//!
//! ## PROB-155: Хранение соли в памяти без очистки
//! Текущая реализация хранит соль в `String` без безопасной очистки памяти.
//! Для CLI-игры это приемлемо — данные не содержат чувствительной информации
//! пользователя. Потенциальное улучшение: использовать crate `zeroize` для
//! безопасной очистки памяти при выходе из программы.
